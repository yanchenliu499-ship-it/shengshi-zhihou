...
export function ScatterChart() {
  ...
  const option = useMemo<echarts.EChartsOption>(() => {
    ...
    const series = periods.map((pd, pi) => ({
      ...
      emphasis: {
        itemStyle: { opacity: 1, borderColor: "#2c2416", borderWidth: 1.5 },
        scale: 1.4,
      },
    }));
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: unknown) => {
          ...
          const p = data?.poem;
          if (!p) return "";
          return `<b>${p.author}</b> · ${p.title}<br/>情感: ${p.sentiment.toFixed(2)} | 正/${p.positive_words} 负/${p.negative_words}<br/><i style="color:#9b8e7a;">点击查看详情</i>`;
        },
      },
      ...
      yAxis: {
        ...
        max: 1.2,
        name: "情感分值",
        nameTextStyle: { color: "#9b8e7a", fontSize: 11 },
