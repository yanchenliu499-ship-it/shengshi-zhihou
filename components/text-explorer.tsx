...
function poemCardHtml(p: TextEntry): string {
  const periodColor = PERIOD_COLORS[PERIOD_ORDER[p.period] ?? 0] ?? "#9b8e7a";
  ...
  return `<div style="width:100%;height:100%;background:#faf7f2;border:1px solid #e0d8c8;border-radius:12px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 4px 14px rgba(44,36,22,0.12);">
  ...
  <div style="padding:10px 12px 8px;flex:1;display:flex;flex-direction:column;gap:4px;min-height:0;">
    <div style="font-family:'Noto Serif SC',serif;font-size:14px;font-weight:700;color:#2c2416;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(title)}</div>
    <div style="font-size:11px;color:#6b5e4a;">${escapeHtml(author)} · ${escapeHtml(period)}</div>
    <div style="font-size:11px;color:${periodColor};">${emoji} ${label}　正${p.positive_words ?? 0} / 负${p.negative_words ?? 0}</div>
    <div style="margin-top:auto;font-size:11px;line-height:1.5;color:#8a7f6d;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">${escapeHtml(first)}</div>
  </div>
</div>`;
}
...
function highlight(text: string, query: string) {
  ...
  return parts.map((part, i) =>
    part.toLowerCase() === query.toLowerCase() ? (
      <mark key={i} className="rounded bg-[rgba(184,134,11,0.25)] px-0.5 text-inherit">
        {part}
      </mark>
    ) : (
      part
...
export function TextExplorer() {
  ...
  return (
    <div>
      {/* 筛选栏 */}
      <div className="mb-6 flex flex-col gap-3 rounded-2xl border border-line bg-card p-4 shadow-[0_2px_12px_rgba(44,36,22,0.08)] md:flex-row md:items-center">
        <select
          aria-label="时期筛选"
          value={period}
      ...
      <div className="mb-6 overflow-hidden rounded-2xl border border-line bg-paper-deep shadow-[0_2px_12px_rgba(44,36,22,0.08)]">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {filtered.map((t) => {
            ...
            return (
              <article
                key={t.id}
                className="rounded-2xl border border-line bg-card p-5 shadow-[0_2px_12px_rgba(44,36,22,0.08)] transition-shadow hover:shadow-[0_8px_32px_rgba(44,36,22,0.12)]"
              >
                ...
                <div className="mt-3 flex flex-wrap gap-2 text-xs">
                  <span className="rounded bg-[rgba(184,134,11,0.12)] px-2 py-0.5 font-medium text-[#b8860b]">
                    正{t.positive_words || 0}
                  </span>
                  <span className="rounded bg-[rgba(139,26,26,0.12)] px-2 py-0.5 font-medium text-[#8b1a1a]">
                    负{t.negative_words || 0}
                  </span>
                  <span className="rounded bg-[rgba(107,94,74,0.1)] px-2 py-0.5 font-medium text-[#6b5e4a]">
